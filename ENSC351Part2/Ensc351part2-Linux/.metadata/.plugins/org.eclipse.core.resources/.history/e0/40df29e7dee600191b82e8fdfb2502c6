...
void termFunc(int termNum)
{
	...
	if (termNum == Term1) {
		char *receiverFileName = "transferredFile-cs";
...
//		receiverFileName = "transferredFile";
...
//		COUT << "xReceiver result was: " << xReceiver.result << endl;
		...
		//20190924 close the socket thrd1 to medium
		std::this_thread::sleep_for (std::chrono::milliseconds(1));
...
//		COUT << "xSender result was: " << xSender.result << endl;
		...
		//20190924 close the socket Medium to thrd2
		std::this_thread::sleep_for (std::chrono::milliseconds(1));
...
void mediumFunc(void)
{
	...
	Medium medium(daSktPrT1M[MediumSkt],daSktPrMT2[MediumSkt], "xmodemData.dat");	//20190924
	medium.run();
}
